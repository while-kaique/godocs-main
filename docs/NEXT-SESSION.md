...
## Plano ativo
**→ [docs/plans/fatia-c-destravar-prod.md](plans/fatia-c-destravar-prod.md)** · Status: **✅ aprovado (Luis, 31/08)** — escopo **G + F + B** (destravar prod), escolhido pelo Luis. Corrige o bug bloqueador da fatia C (cabeçalho "Documentação Técnica" sobre o memorial de saving no fim do fluxo reordenado — ramo `else` de `submeter.tsx:2583-2597` vira `chatFase` para `"doc"` sem limpar as mensagens), decide o `reasoning_effort=low` e rotula o loader ("gerando o memorial…"). Worktree `~/godocs-wt-fatia-c` (branch `feat/fatia-c-reorder-wizard`, HEAD `a3d6acb`, suíte 2346 verde). **D/E/C/A ficam FORA** (frente sensível — mexem nos gates que protegem os números; planejar à parte). Captura completa dos issues A-G: [fatia-c-polish-agente-latencia.md](plans/fatia-c-polish-agente-latencia.md). Memória: `frente1-fatiac-reordenar-wizard`.
