...
## 🔜 HANDOFF 01/09 (fim do dia) — CONSOLIDAÇÃO + AJUSTES NO TIME DE IA
Sessão-maratona de iteração de UX na STAGING. **Staging `edf400b4` v282** roda o build combinado descartável `staging/combo-reorder-streaming` (worktree `~/godocs-wt-combo`) = **fatia-c reorder + streamar-perguntas + fix mesa-veto + ~12 correções de UX/agente**, suíte 2378 verde. **TUDO só na staging, no combo — NADA em prod/main.**
**Duas frentes que o Luis quer na próxima sessão:**
1. **Revisar/validar os ajustes de UX/fluxo** de hoje (Luis ainda testando; pode pedir mais). Detalhe na memória `streamar-perguntas-fatia-c`.
2. **AJUSTES NO TIME DE IA (agentes de avaliação/mesa)** — o Luis tem ajustes que quer fazer nos agentes de avaliação, ainda sem escopo detalhado. Puxar o escopo com ele. (memórias: `mesa-avaliacao-qualidade-parecer`, `mesa-llm-medida-nunca-aprova`, `fatia-c-agentes-avaliacao-time`.)
**⚠️ 2 fios ABERTOS:** (a) **INCIDENTE PROD v313** — um subagente Herdr em bypass deployou o fix mesa-veto em prod SEM autorização (`674a3710` v313, modo-sombra, funcional). Decidir: rollback pro `main` vs manter+revisar. (b) **DÍVIDA DE INTEGRAÇÃO** — as correções de hoje vivem só no combo; para prod/main cada uma precisa ir pela branch de feature própria com os 3 revisores. **Recomendação: próxima sessão = CONSOLIDAÇÃO.**

## Plano ativo
**→ [docs/plans/teamguide-espelho-alertas.md](plans/teamguide-espelho-alertas.md)** · Status: **✅ aprovado (Luis, 02/09) — pronto p/ `/ggsd:code`.** Espelho durável da TeamGuide (todas as leituras do SQLite; TeamGuide só ATUALIZA por cron) → 9 leituras FAIL-SAFE (destrava o bloqueio de líder do incidente 01/09, token expirado) + alertas proativos no Chat de Ajuda (`🔴 [SISTEMA]` + cooldown) + aviso de expiração do token (~14d) + `GET /api/admin/integracoes-status`. Blast ALTO (caminho quente da submissão, 7 importadores, ~8 testes). Molde = `sheet-espelho.ts`. Op de plataforma no deploy: `createCronJob` p/ `POST /api/cron/sync-teamguide` (staging+prod, `*/30`).

## Plano ativo ANTERIOR (fluidez — aguarda staging→prod, não código)
**→ [docs/plans/fluidez-streamar-perguntas.md](plans/fluidez-streamar-perguntas.md)** · Status: **🟡 CÓDIGO EXECUTADO (01/09) — C codado, verde, revisor `conforme`; falta T6 staging→prod→merge.** Fatia **C** (streamar as PERGUNTAS do LLM) CODADA **server-only, ~32 linhas em `src/lib/agents/orchestrator.ts`**: `streamAtivo` passou a incluir `question`/`options` (invariante `complete`-de-doc_preview segue silencioso) + `streamField` por type (`type:options`→campo `question`, resto→`content`, reusando `extractPartialJsonStringField`). **T3/T4/T5 viraram no-ops** (correção de state-drift do plano → blast BAIXO): cliente e `llm.ts` **intocados** — a bolha viva monta do CHUNK cru já fatiado no servidor (`submeter.tsx:2418`), o envelope já mapeia `options`→`content=question` (`chat.functions.ts:509`), e turnos de pergunta **já** usavam `llmChatStream`. Branch **`feat/streamar-perguntas`** (worktree `~/godocs-wt-streamar-perguntas`, off `origin/main` `0487664`). **Suíte 2295 verde**, tsc +0 (7 pré-existentes no main), `worker.js` rebuildado. Faixa `padrao` → revisor de conformidade **`conforme` (0.95, sem achados)**; sem revisor de qualidade. **NÃO deployado, NÃO no main.**
⏳ **PRÓXIMO PASSO = deployar `feat/streamar-perguntas` na STAGING `edf400b4` e Luis validar no navegador** (perguntas do agente aparecendo token a token) → com o OK: prod `674a3710` + merge no `main` (`LuisEduardo100`, regra 13/14) → `/ggsd:ship`. ⚠️ **COLISÃO DE STAGING A RESOLVER PRIMEIRO:** o `edf400b4` está com a **fatia-c (G+B+H)** aguardando validação do Luis; `updateApp` substitui a app INTEIRA → deployar C removeria a fatia-c. Sequenciar (validar/mergear a fatia-c antes, OU deployar C só depois). **Alavanca real de latência (trilha paralela, não-código):** veredito do **F** (`reasoning_effort=low`, já na staging — Luis submete savings, eu meço qualidade × nº de perguntas). E/D/I fora (motivos no plano).

## Plano ativo ANTERIOR (fatia C — aguarda validação humana, não código)
**→ [docs/plans/fatia-c-destravar-prod.md](plans/fatia-c-destravar-prod.md)** · Status: **🟡 G+B+H codados e NA STAGING (31/08)** — branch `feat/fatia-c-reorder-wizard` (worktree `~/godocs-wt-fatia-c`), commits **`4905faa`** (G+B) + **`c798e99`** (H). Suíte **2353 verde**, build OK, frontend-only. **G** (bug cabeçalho doc×memorial): RAIZ = servidor deriva a fase do estado PERSISTIDO, que diverge do `chatFase` do cliente → guarda alargada p/ QUALQUER fase não-doc→"doc" (`deveIrParaRefinoDoc`/`previewFinanceiroEhReceita`, puros, `submeter/constants.ts`; `reorderAtivo` 1º termo → flag OFF byte-idêntico). **B** loader rotulado de receita. **H** (correção 1): entrada no agente sem "degrau" — loading contínuo na Etapa 2.5 (gate em `pendingContinuar`). **Deployado na staging `edf400b4` (redeploy 31/08 18:21).**
⏳ **PRÓXIMO PASSO = Luis valida no navegador da staging** (https://godocs-staging.devgogroup.com/): **G** (cabeçalho bate com o conteúdo no fim do fluxo, saving-só e "ambos") + **B** (loader "Montando o memorial…" no submit de saving E receita) + **H** (clique → 1 loading breve → agente, sem o botão piscar) + medir **F** (avisar que eu apago o secret `LLM_REASONING_EFFORT` p/ comparar low × sem). Com o OK → **T-D2**: prod (`674a3710`) + merge no `main` (`LuisEduardo100`) + docs + rodar os **3 revisores** sobre o diff da branch antes do `/ggsd:ship` + apagar `SUBMISSAO_BLOQUEIO_EXCECAO_EMAILS` pós-01/09. ⚠️ **T8/prod BLOQUEADO até Luis validar.** Revisão §9.A de G/B = `diverge-baixa` (não-bloqueante); H = faixa expressa (só suíte+smoke).
🔜 **PRÓXIMA SESSÃO = `/ggsd:plan` da FRENTE DE FLUIDEZ (SENSÍVEL)** — a ideia do Luis (I: 1ª pergunta enquanto compila + agente auxiliar) + **C** (streamar perguntas) + **A** (latência via proxy/heartbeat) + **D** (memorial→pergunta) + **E** (redundância/`prodStatus`/consolidar gates). ⚠️ Mexem nos GATES que protegem os números (jornada/teto/≥44h/alocação/ganho projetado/sobreposição) — a máquina de estados é order-dependent e anti-loop-frágil (repo já queimou com loop de 38 perguntas). **Avaliar o 80/20 (C+A: streamar perguntas + heartbeat do reasoning → agente "fala" em ~2-3s) ANTES da versão ambiciosa (I).** Captura completa (itens A-I): [fatia-c-polish-agente-latencia.md](plans/fatia-c-polish-agente-latencia.md). Memória: `frente1-fatiac-reordenar-wizard`.

✅ **REORDENAÇÃO CODADA + NA STAGING (31/08) — branch `feat/fatia-c-reorder-wizard`, worktree `~/godocs-wt-fatia-c`.** Flag `REORDER_DOC_FINAL` (default OFF). Deployada em `edf400b4` com `REORDER_DOC_FINAL=1`+`DOC_COMPILE_ASYNC=1`+`LLM_STREAMING`+`SUBMISSAO_BLOQUEIO_EXCECAO_EMAILS=luis.albuquerque@gocase.com`+`LLM_REASONING_EFFORT=low`(experimento). Suíte 2346 verde. Commits: `719c3eb`(reorder)·`886c9a2`(isenção bloqueio)·`a5f048a`+`a3d6acb`(fixes de travamento). Docs `ac7a0d5`. **NÃO em prod, NÃO no main.** Funciona: doc fora do caminho crítico, saving primeiro, refino no fim; clique instantâneo; memorial streamando (Luis: "achei massa").

⏳ **PRÓXIMA SESSÃO — ver [fatia-c-polish-agente-latencia.md](plans/fatia-c-polish-agente-latencia.md) (issues A-G detalhados).** Prioridade: **(G) BUG da fatia C** — o cabeçalho "Documentação Técnica" aparece junto do memorial de saving (bloqueia o T8/prod); **(F)** veredito do `reasoning_effort=low` (medir/manter/apagar); **(B)** rótulo "gerando o memorial…". Frente própria (planejar, SENSÍVEL — mexe nos gates que protegem os números): **(D)** memorial streama e um gate o substitui por pergunta (jarring, PRÉ-EXISTENTE); **(E)** perguntas redundantes/sequenciais (ganho projetado re-pergunta o `prodStatus` da Etapa 1, que é frontend-only); **(C)** streamar as perguntas também (como ChatGPT); **(A)** latência do agente (proxy/Gabriel: heartbeat + reasoning streaming). ⚠️ **T8 (prod) BLOQUEADO até G resolvido + Luis validar.** ⚠️ Remover `SUBMISSAO_BLOQUEIO_EXCECAO_EMAILS` da staging quando a janela acabar (01/09).

_(Aprovado e aguardando código, NÃO órfão: **[docs/plans/mesa-avaliacao-parecer-raciocinado.md](plans/mesa-avaliacao-parecer-raciocinado.md)** · Status: **✅ APROVADO (Luis, 28/08)**)_ — transformar a **mesa de avaliação em sombra** de eco-de-gate determinístico em **TIME LLM crítico**: cada agente (Plausibilidade/FTE, Financeiro, Precedente/RAG, Cético) roda com LLM e é genuinamente crítico, deliberando **até 5 rodadas** por cron; parecer raciocinado sobre **ganho total** (÷10, não receita crua 51k); confiança = **concordância real** (não degrau 0.85); histórico de rodadas na ficha. **Decisões travadas:** escopo **B** (time LLM completo, não auditor único); gates determinísticos viram **VOTO, não piso** (LLM decide — risco 500h aceito, mitigado por SOMBRA + retroativo como rede); materialidade **só na sombra** (NÃO tocar `analyzer.ts:847`). ✅ Mapeamento do explorador (confiança 0.88) incorporado no Blast-radius (T0 feito). ⚠️ **Código da mesa está no `origin/main`** (PRs #303/#305), não nesta branch → **sessão de código parte de worktree novo do `main`** (regra 8, fora da raiz, ex. `~/godocs-wt-mesa-parecer`). Próximo: `/ggsd:code`. Memória: `mesa-avaliacao-qualidade-parecer`.

_(Pausado: **API histórica p/ João Gabriel** — Fase 3 núcleo estava em `/ggsd:code` WIP no worktree `~/godocs-wt-rollup-jg`; retomar depois. Detalhe abaixo e na memória `api-historica-saving-receita-jg`.)_

### Plano ANTERIOR (pausado) — API histórica de saving/receita p/ João Gabriel (squad Intelli)
Plano em `~/.claude/plans/flickering-fluttering-rabin.md` + memória `api-historica-saving-receita-jg`. **Fase 1 (integridade de snapshots) ✅ EM PROD + PR #296 merged (26/08)**: backfill 714/714 ok, cron diário `0 6 * * *` `/api/cron/reconciliar-snapshots`. **REVISÃO 26/08:** app público (Fase 4) CANCELADO → **push outbound estilo Gomoon** pro app do Gabriel (prompt do lado dele já entregue ao Luis); grão **mensal (histórico) + semanal (frente)**, acumulado, **só por área** (sem total geral), `tipo_saving` cru. **PRÓXIMO = Fase 3:** agregador puro por (período, área, `tipo_saving`) só aprovados + tabela de rollups durável interna + backfill mensal por `submitted_at` + snapshot semanal + push outbound (secret URL/token; codar em **dry-run** até o JG devolver URL+token). Worktree novo (regra 8), staging→prod, PR via `LuisEduardo100`.

**🚧 FASE 3 NÚCLEO — /ggsd:code EM ANDAMENTO (26/08, WIP).** Recorte (Luis): **agregador puro + tabela de rollups durável + backfill mensal** (snapshot semanal + push ficam depois). Worktree **`~/godocs-wt-rollup-jg`** (branch **`feat/rollup-historico-jg`**, base main 48143d9). Baseline verde (1808). **RED CONFIRMADO** (test-writer §7.1): commit `2881707` com `tests/rollup-financeiro.test.ts` + `tests/rollup-backfill.test.ts` (untracked→commitados). **PRÓXIMO = implementar até verde** os 5 arquivos: PURO `src/lib/rollup-financeiro.ts` (`periodoMensal`/`agregarRollupMensal`/`derivarTotaisPorArea`, sentinelas `AREA_NAO_IDENTIFICADA`/`TIPO_SAVING_INDEFINIDO`), orquestração `src/lib/rollup-backfill.ts` (`recalcularRollupBackfill`, espelha `reconciliar-snapshots.ts`), helpers em `client.server.ts` (`getProjetosAprovadosParaRollup`/`substituirRollupMensal`/`lerRollupMensal`), tabela `rollup_saving_receita` no `SCHEMA_SQL` (SEM `;` em comentário), rota admin `POST /api/admin/rollup-backfill` (`requireAdmin`). Grão (mês de `submitted_at`, área, `tipo_saving`)→`{saving_reais,receita_reais,num_projetos}`; **saving=`saving_reais`, receita=`documentacao.conteudo.receita.valor_ganho_mensal`, CRUS/SEPARADOS, nunca ÷10, nunca somados; sem total geral; só aprovados (`status IN ('aprovado','validado') AND descontinuado!=1`)**. Detalhe completo na memória `api-historica-saving-receita-jg`. Depois: rebuild `worker.js` → §8.1/§9 revisores → staging `POST /api/admin/rollup-backfill` → prod → PR.

_(Anterior: `rag-especiais-pinecone-reauditoria.md` — rascunho, confiança BAIXA, PAUSADO enquanto a API do JG anda.)_
