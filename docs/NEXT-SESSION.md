...
**Esta sessão (26/08, cont.):** adicionei a **rota de leitura `GET /api/admin/rollup-mensal`** (commit `71c8b84`, worktree `~/godocs-wt-rollup-jg`, branch `feat/rollup-historico-jg`; suíte **1824 verde**) — devolve células + totais por área + resumo `verificacao` (soma global, só apoio de admin, NÃO é contrato do JG). Deployei staging (v223) E prod (v296, prod==main verificado), rodei backfill idempotente e conferi nos dois. Consistência interna OK. Receita ≈ **R$139** é REALIDADE da prod (extração correta: `valor_ganho_mensal` gravado como NÚMERO; só 1 projeto tipado receita em 838 e com valor nulo).

⛔ **BLOQUEADOR — decisão do Luis pendente (NÃO assumir).** O cross-check de saving NÃO bateu: rollup filtra por `projetos.status IN ('aprovado','validado')` = **286 proj / R$196.184,67**, mas o dashboard usa a coluna "Status" do **espelho** = **643 "Aprovado" / R$1.351.340,84** (7x). Causa-raiz: o sync reverso **exclui `status`** → aprovações da TRIAGEM (na planilha) nunca voltam pro `projetos.status`. Contagem fecha (838 − 124 rascunhos = 714 = espelho; nenhum projeto some — diverge só o VALOR do status, 2 campos separados). Luis vai decidir QUAL "aprovado" a história do JG usa. Se **planilha (643)** → mudar `getProjetosAprovadosParaRollup` (`client.server.ts:680`) pra filtrar pelo `sheet_espelho` "Status"='Aprovado' (join `saving_reais` por id) + teste + recomputar backfill. Se **SQLite (286)** → nada muda. Rollup na prod está DORMENTE (nada consome), inócuo até recomputar. **PR SEGURADO até a decisão.**

⚠️ **RESSALVA — revisão GGSD (§9) NÃO rodou.** `/ggsd:ship` barra até rodar (marcador `.review-status` ausente).

**Próximo passo:** obter a decisão do Luis sobre a definição de "aprovado" (planilha/643 vs SQLite/286) → ajustar o filtro do rollup + recomputar backfill (se planilha) OU seguir direto (se SQLite) → rodar revisores GGSD (§9) → PR via `LuisEduardo100`. Depois (sessões seguintes): snapshot semanal (cron) + push outbound (dry-run até `JG_INGEST_URL`; `JG_INGEST_TOKEN` já setado).
