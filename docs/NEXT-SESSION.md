...
### ✅ T1 IMPLEMENTADO E VERDE (28/08) — próximo é **T2**
**T1 fechado:** os 2 módulos NOVOS existem e o RED virou verde. `src/lib/agents/especialista-avaliacao.ts` (PURO: tipos `DimensaoAvaliacao`/`TextoProjeto`/`VotoDeterministico`/`VotoResumido`/`EntradaEspecialista`/`JulgamentoEspecialista`; `fallbackDeterministico`; `buildPromptEspecialista` — persona por dimensão, cético ADVERSARIAL; `normalizarJulgamento` fail-closed + confiança clampada 0..1, `confiança 'abc'/NaN → cai no voto`) + `especialista-avaliacao.functions.ts` (`especialistasMesaLlmLigados()` env `AVALIACAO_MESA_LLM` LAZY OFF, separada de `AVALIACAO_NORMAIS`; `julgarComEspecialista()` llmChat jsonMode → `extrairJson` → normaliza, **NUNCA lança** → fallback). `tests/especialista-avaliacao.test.ts` **24 verde**; suíte cheia **2256 verde**; `tsc` real sem erro nos novos. Nada toca produção (nenhum arquivo os importa ainda).

✅ **REVISÃO (§9) RODADA — NÃO BARRA:** conformidade = **`conforme`** (0.85, "faz o que o plano diz, nada além, produção intocada"); qualidade = **`sugestoes`** (0.83, não-bloqueante). Os 2 achados baixos da qualidade foram **APLICADOS** (commit de acompanhamento): (a) `preocupa` malformado agora cai no fallback em vez de coagir para `false` (fail-closed na direção segura); (b) comentário "LLM leve" corrigido — a chamada é neutra, roteamento de modelo da mesa é decisão do T5. Marcadores `.review-status=conforme`/`.quality-status=sugestoes`. **Alerta herdado p/ T5:** garantir que `voto.motivo` (input do prompt) nunca carregue a tarifa oculta valor/hora por cargo (o invariante "sem R$ cru"); T1 não a fabrica.

**PRÓXIMO = T2 — Agregador/chair sobre os votos LLM.** `agregarVotos` (`src/lib/agents/agregador-avaliacao.ts`) passa a conciliar os `JulgamentoEspecialista` (LLM) + os determinísticos como votos; **confiança = concordância real** entre agentes (dispersão/consenso), fim do degrau fixo 0.85 (`agregador-avaliacao.ts:58`). Invariantes: nunca `aprovar` negativo automático; especial/liderança isentos; sombra. RED primeiro. Depois T3 (deliberação 2→5 + histórico APPEND) → T4 (materialidade `saving+receita/10` em `avaliacao-normais.functions.ts:290-307`, NÃO tocar `analyzer.ts:847`) → T5 (orquestração/persistência) → T6 (UI rodadas) → T7 (retroativo = rede) → §8.1/§9 → staging → prod → PR.

### (histórico) 🚧 T1 estava em RED (28/08, WIP, contexto estourou no meio)
