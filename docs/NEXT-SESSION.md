...
⚠️ **RESSALVA — revisão GGSD (§9) NÃO rodou.** `/ggsd:ship` barra até rodar.

**Próximo passo (DECISÃO do Luis, pull×push):** o Luis pediu "que o Gabriel possa PUXAR as infos quando quiser". Isso REVERTE a decisão documentada de PUSH ("app público CANCELADO, GoDocs empurra"). Pull ⇒ app PÚBLICO novo `godocs-metrics-api` (edge de godocs dá 302; cada app tem env.DB isolada → godocs EMPURRA o rollup pra ele e o Gabriel lê com READ key); push ⇒ godocs manda pro endpoint do Gabriel. **CONFIRMAR pull×push antes de construir.** Depois: cron pra manter o rollup fresco + **PR do rollup atual pra main (regra 14 — prod já está à frente)** + CLAUDE.md/spec.
