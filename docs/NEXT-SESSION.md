...
## Plano ativo
**→ [docs/plans/fatia-c-destravar-prod.md](plans/fatia-c-destravar-prod.md)** · Status: **🟡 executado parcial (G+B codados, 31/08)** — commit **`4905faa`** na branch `feat/fatia-c-reorder-wizard` (worktree `~/godocs-wt-fatia-c`). **G (bug do cabeçalho doc×memorial) + B (loader rotulado de receita) prontos e testados** (suíte **2353 verde**, build OK, frontend-only → sem worker rebuild). **RAIZ de G confirmada (T-G1):** o servidor deriva a fase do estado PERSISTIDO, que pode DIVERGIR do `chatFase` do cliente (o preview chega ainda como `saving`/`receita` cru) → a guarda antiga escapava para o `else` (`submeter.tsx:2596`) que virava `chatFase→"doc"` sem limpar. Fix = 2 funções PURAS em `submeter/constants.ts` (`deveIrParaRefinoDoc` — QUALQUER fase não-doc→"doc" no reorder limpa atômica; `previewFinanceiroEhReceita`), `reorderAtivo` 1º termo → flag OFF byte-idêntico.
⏳ **PRÓXIMO PASSO = T-D1 (regra 13):** deploy da branch na **staging** (`edf400b4`, secrets `REORDER_DOC_FINAL=1`+`DOC_COMPILE_ASYNC=1`+`LLM_STREAMING`) → **Luis valida no navegador**: cabeçalho bate com o conteúdo no fim do fluxo reordenado (G) + loader "Montando o memorial…" no submit de saving E receita (B) + smoke do loader de receita com streaming OFF (achado 2 do revisor, não-bloqueante) → medir **F** (1 saving `LLM_REASONING_EFFORT=low` × 1 sem: qualidade do memorial + nº de perguntas → manter/apagar). Depois **T-D2**: prod (`674a3710`) + merge no `main` (`LuisEduardo100`) + docs + rodar os **3 revisores** sobre o diff da branch antes do `/ggsd:ship` + apagar `SUBMISSAO_BLOQUEIO_EXCECAO_EMAILS` pós-01/09. ⚠️ **T8/prod BLOQUEADO até Luis validar G.** Revisão §9.A desta sessão = `diverge-baixa` (não-bloqueante). **D/E/C/A ficam FORA** (frente sensível — planejar à parte). Captura dos issues A-G: [fatia-c-polish-agente-latencia.md](plans/fatia-c-polish-agente-latencia.md). Memória: `frente1-fatiac-reordenar-wizard`.
