...
## Plano ativo
**→ [docs/plans/fatia-c-reordenar-wizard-doc.md](plans/fatia-c-reordenar-wizard-doc.md)** · Status: **aprovado (Luis: "codar"), aguardando 1 decisão de UX antes de codar** — reordenar o wizard da submissão (doc paralela, refino no FINAL). Decisão de produto FECHADA (Luis, 29/08: doc compila em bg desde o anexo → saving/receita primeiro com todos os gates → refino conversacional da doc por último → submit). Plano completo (T1–T8, Blast-radius ALTO conf 0.72; destravador confirmado em código). Memória: `frente1-fatiac-reordenar-wizard`.

✅ **AMBIENTE PRONTO (31/08):** worktree **`~/godocs-wt-fatia-c`** (branch **`feat/fatia-c-reorder-wizard`**, criado de **`origin/main`** `0487664` — a branch docs é stale, `doc-async.ts` não existe nela), `node_modules` symlink, **baseline VERDE 2293/2293**. Nada codado.

✅ **2 DECISÕES DE ENGENHARIA RESOLVIDAS (grounded em código):** (1) a reordenação vai atrás de **flag env LAZY** (default OFF = byte-idêntico, padrão do `DOC_COMPILE_ASYNC`) — neutraliza o risco da máquina de fases ser compartilhada; threadar `reorderDocFinal` no `streamOpts` de `runOrchestrator`. (2) o `coletadoInicial` é guardado como **`coletado_inicial` no blob da doc** (sobrevive à compilação bg via o `resto` de `mergeDocCompilada`); `iniciarSaving` o relê quando o chat não tem coletado — confirmado que `buildSavingPrompt` USA `coletado` via `buildDetalhesAprovados`.

⏳ **1 DECISÃO DE UX PENDENTE (product-visible):** formato do refino da doc no fim — perguntei (A) card na revisão final + "Pedir ajustes" opcional que abre chat [recomendado] × (B) fase de chat de doc SEMPRE no fim. **Luis rejeitou a pergunta querendo ESCLARECER algo antes** → NÃO assumir o formato. **Próximo passo: perguntar ao Luis o que ele quer esclarecer sobre o refino, travar o formato, então `/ggsd:code` no worktree `~/godocs-wt-fatia-c`.**
