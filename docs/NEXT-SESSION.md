...
⚠️ **RESSALVA DE REVISÃO (§9) — PENDENTE:** os 2 revisores GGSD (`ggsd:verificador-conformidade` + `ggsd:revisor-qualidade`) foram DISPARADOS sobre o diff do T1 mas a sessão fechou (contexto ~92%) ANTES dos vereditos. Marcadores `.review-status`/`.quality-status` = **`pendente`**. O commit passou (branch é reversível), mas **`/ggsd:ship` VAI BARRAR** até rodar a revisão. Destravar: reabrir `/ggsd:code` (ou revisar o diff) e gravar os marcadores verdes ANTES de enviar. Os vereditos podem estar no transcript da sessão de 28/08 se as tarefas de background completaram.
