...
## Plano ativo
**→ [docs/plans/fatia-c-reordenar-wizard-doc.md](plans/fatia-c-reordenar-wizard-doc.md)** · Status: **aprovado — CÓDIGO COMPLETO (T1–T6), falta DEPLOY (T7 staging · T8 prod+merge)** — reordenar o wizard da submissão (doc paralela, refino no FINAL). Memória: `frente1-fatiac-reordenar-wizard`.

✅ **/ggsd:code COMPLETO (31/08) — worktree `~/godocs-wt-fatia-c` (branch `feat/fatia-c-reorder-wizard`).** Tudo atrás da flag **`REORDER_DOC_FINAL`** (env lazy, default OFF = byte-idêntico). Backend: `proximaFase` pura (mapa de transição, unifica 2 blocos), ramo reorder de `iniciarSubmissao` (bg-compile + `coletado_inicial` durável no blob), endpoint `iniciar-refino-doc` (refino no fim, condicional), recompile só no delta (`coletadoIgual`). Frontend `submeter.tsx`: `reorderAtivo`, abre form saving direto, `transitionToDocRefino` com streaming. **UX travada com Luis (31/08):** o refino é o comportamento de HOJE reposicionado (doc completa → card de aprovação; incompleta → agente pergunta). **Suíte 2346 verde.** §9 revisores tratados (review=diverge-baixa não-bloqueio, quality=limpo). `worker.js` rebuildado. Commitado na branch, NÃO na main.

⏳ **PRÓXIMO — T7 (staging, regra 13, precisa do Luis):** deploy no **`edf400b4`** → secrets **`REORDER_DOC_FINAL=1` + `DOC_COMPILE_ASYNC=1`** (LLM_STREAMING já on) → Luis valida no navegador (submissão nova padrão: doc some do caminho crítico, saving/receita primeiro, refino no fim) + medir TTFT + doc íntegra no Drive/analisador. Depois **T8 prod `674a3710`** + secrets + merge no `main` (`LuisEduardo100`) + CLAUDE.md/specs. **T5 demo-backend** (dev-only) fica p/ o deploy da flag. ⚠️ review=diverge-baixa NÃO barra o `/ggsd:ship`.
