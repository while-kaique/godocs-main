...
| [dashboard-admin-sheets](dashboard-admin-sheets.md) ◀ | ✅ executado (2026-07-28) — falta T8 (staging→prod→PR) | Dashboard do admin vira triagem sobre a **planilha** (lia SQLite → mostrava rascunho e status errado): busca/filas/paginação, ficha com todas as colunas e mudança de status gravando no Sheets + auditoria `admin_status_log` |
