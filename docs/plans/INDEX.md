...
| [criterios-projeto-classificacao](criterios-projeto-classificacao.md) ◀ | ✅ **executado** (2026-07-29 — T1–T7 + refinamento R1/R2 + **R3: contexto do formulário nos 4 prompts**, `53e8ef8`) — falta **deployar no staging** (lá está o `b6485e4`) e rodar os 8 cenários de `docs/roteiro-validacao-criterios.md` → prod → PR (+ calibrar a régua com o Rafa) | Critério de projeto (nota do Rafa): na Etapa 2 fica só o **contrafactual** — "quem reclama" por **seleção da Team Guide (pessoa OU time inteiro)** + "o que piora"; **ponteiro movido + onde verificar e "que processo mudou e quanto" ficam com o AGENTE** (seções `[1.3]`/`[1.4]` do memorial); analisador classifica **claro sim / claro não / zona cinzenta** com justificativa SEMPRE; `claro não` → **Reprovado** na planilha + `Motivo Reprovado`; colunas novas `Classificação`/`Motivo Reprovado`/`Motivo Reenvio`; **formulário continua não barrando** |
