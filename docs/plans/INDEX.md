...
| [loadings-dashboard-admin](loadings-dashboard-admin.md) ◀ | rascunho | Tirar a espera percebida do `/dashboard`: stale-while-revalidate no servidor, cache de auth em `sessionStorage`, leitura da planilha em paralelo com o auth e skeleton no lugar do spinner (sem cache em SQLite — planilha segue fonte única) |
| [dashboard-admin-sheets](dashboard-admin-sheets.md) | ✅ executado (2026-07-28) — staging+prod deployados, PR aberto | Dashboard do admin vira triagem sobre a **planilha** (lia SQLite → mostrava rascunho e status errado): busca/filas/paginação, ficha com todas as colunas e mudança de status gravando no Sheets + auditoria `admin_status_log` |
| [perguntas-agente-recorrencia-evidencia](perguntas-agente-recorrencia-evidencia.md) | **rascunho** (aguarda OK do Luis) — T1 já executado (2026-07-28) | Cortar a redundância das perguntas do agente e embutir recorrência/contrafactual/rastreabilidade (nota do Rafa) **sem** criar gate que barre; baseline medido em 24 conversas reais → `docs/analise-perguntas-agente.md` |
