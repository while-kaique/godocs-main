...
| [loadings-dashboard-admin](loadings-dashboard-admin.md) ◀ | ✅ **aprovado** (Luis, 2026-07-28) — pronto para `/ggsd:code` | Tirar a espera percebida do `/dashboard`: stale-while-revalidate no servidor, cache de auth em `sessionStorage`, leitura da planilha em paralelo com o auth e skeleton no lugar do spinner (sem cache em SQLite — planilha segue fonte única) |
