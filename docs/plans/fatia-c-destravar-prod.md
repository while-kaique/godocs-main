...
**Status:** ✅ aprovado (Luis, 2026-08-31) — G+B já codados (commit `4905faa`); **item H (correção 1) aprovado para codar nesta continuação** (Luis deu o go explícito após o teste na staging). Branch `feat/fatia-c-reorder-wizard` (worktree `~/godocs-wt-fatia-c`). **Falta (não é código):** F (veredito `reasoning_effort=low`, medição na staging — T-F1) · T-D1 (deploy staging + Luis valida G/B/H no navegador) · T-D2 (prod + merge + docs + 3 revisores sobre o diff da branch + apagar `SUBMISSAO_BLOQUEIO_EXCECAO_EMAILS` pós-01/09). Revisão §9.A de G/B = `diverge-baixa` (não-bloqueante).

### Tarefa H — entrada no agente "degrau" (correção 1, reorder)
- ✅ **T-H1 CODADO** (commit `c798e99`, 31/08, faixa expressa): no fluxo reordenado, o clique em "Analisar com Agente" (Etapa 2.5) esperava o bg-compile, DESLIGAVA o loading (`setIniciandoChat(false)`) e só marcava `pendingContinuar` → o botão re-renderizava como "Continuar com Agente" ESTÁTICO por um beat antes de um `useEffect` navegar pro chat (parecia bugado). Fix: **loading contínuo** do clique até a navegação — o botão "Continuar com Agente" (`submeter.tsx:3519-3534`) trava e mostra loading também enquanto `pendingContinuar` (não só `continuando`), reusando `LOADING_STEPS_INICIAR` na entrada nova (continuidade com a 1ª fase). Sem componente novo; presentation/interação. Suíte 2353 verde, build OK. **Deployado na staging (`edf400b4`) 31/08 junto com G/B — falta Luis validar no navegador.**
