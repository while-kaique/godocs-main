...
### H — Entrada no agente com "degrau" (correção 1, 31/08) — ✅ CODADO
No reorder, "Analisar com Agente" (Etapa 2.5) esperava o bg-compile, desligava o loading e piscava o botão de "Analisar" → "Continuar com Agente" ESTÁTICO antes de navegar (parecia bugado). ✅ Fix `c798e99` (loading contínuo até a navegação; gate em `pendingContinuar`). Na staging; falta Luis validar.

### I — IDEIA DO LUIS (31/08): primeira pergunta ENQUANTO compila + agente auxiliar (a versão ambiciosa de C/A)
Como a compilação já ficou rápida e o que sobra é o TTFB do LLM (A) + perguntas não-streamadas (C), o Luis propôs: **o agente já faz uma 1ª pergunta** e o cliente começa a iterar no chat enquanto, em background (visível no topo: "compilando…"), a doc termina; um **agente auxiliar** processa assincronamente as respostas das primeiras perguntas até o agente principal ter contexto abrangente ("perguntas de ponta"). ⚠️ **FRENTE SENSÍVEL — planejar com /ggsd:plan, medir risco no ESTADO DOS GATES:** os gates (jornada/teto/≥44h/alocação/ganho projetado/sobreposição) rodam pós-orquestrador e dependem da ORDEM e do estado coletado; perguntar fora de ordem + fundir respostas de um 2º agente pode dessincronizar a máquina de estados (repo já queimou com loop de 38 perguntas). **80/20 de menor risco a avaliar no mesmo plano:** streamar as perguntas (C) + heartbeat/streaming do reasoning no proxy (A) → agente "fala" em ~2-3s sem mexer na ordem dos gates. Decidir o recorte no plano.

## Recorte sugerido para a PRÓXIMA sessão (prioridade)
- ✅ **G, B, H já CODADOS e na staging (31/08)** — falta Luis validar no navegador (G/B/H) + medir **F** → então prod (T-D2). Ver [fatia-c-destravar-prod.md](fatia-c-destravar-prod.md).
- **PRÓXIMA sessão = `/ggsd:plan` da FRENTE DE FLUIDEZ (SENSÍVEL):** itens **I** (ideia do Luis: 1ª pergunta enquanto compila + auxiliar), **C** (streamar perguntas), **A** (latência via proxy/heartbeat), **D** (memorial→pergunta), **E** (redundância / `prodStatus` da Etapa 1 já é frontend-only / consolidar gates). ⚠️ Mexem nos GATES que protegem os números — não reescrever no susto; cada mudança com teste + validação de qualidade na staging. Avaliar primeiro o 80/20 (C+A) antes da versão ambiciosa (I).
