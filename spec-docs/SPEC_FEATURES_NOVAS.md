...
- **Espaço deixou de ser separador universal:** no `ChipsInput`, espaço só "fecha" o chip quando o
  texto já é um e-mail completo (`EMAIL_RE`); senão passa como texto — sem isso seria impossível
  buscar por nome composto ("maria souza"). Vírgula/`;`/Tab/Enter seguem separando.
...
- `src/lib/submeter/form-components.tsx` — `ChipsInput` vira combobox (prop `suggestions`).
- `src/lib/submeter/step1.tsx` — carrega quando `emEquipe === 'sim'` e injeta no `ChipsInput`.
