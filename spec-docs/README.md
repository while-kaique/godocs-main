...
| [SPEC_DASHBOARD_ADMIN.md](SPEC_DASHBOARD_ADMIN.md) | **Dashboard do admin = triagem sobre a planilha** (jul/2026, ✅ implementado · ⏳ staging): a tela lia SQLite (mostrava rascunho e status errado) e passa a ler `readAllRows()`; busca instantânea por projeto/autor, filas de status com contagem, paginação, overlay com todas as colunas e **mudança de status gravando no Sheets** + auditoria `admin_status_log`. Decisões D1–D8 — em especial **"Atualizado Em" é intocável**. |
...
| [SPEC_STAGING.md](SPEC_STAGING.md) | **Ambiente de Staging + fluxo de promoção** (jun/2026, ✅ ambiente NO AR · trava dura pendente): app Godeploy `godocs-staging` (`edf400b4`) isolado — **aba `STAGING`** na mesma planilha, pasta de Drive própria, Chat mudo, SQLite próprio (modelo leve, D2 revisada). Guard que recusa defaults de prod + banner visual + `/api/config`. Verificado por E2E. Falta a trava (CI + branch protection — admin do repo); até lá a **regra 13 do `CLAUDE.md`** vale. |
