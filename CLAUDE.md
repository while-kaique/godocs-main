...
- **Legado pendente** — projeto **submetido** (não-rascunho) cuja coluna **AH "Atualizado Em"** está vazia/`—` no Sheets = legado que o app nunca reescreveu → precisa ser editado/reenviado para regularizar (a edição preenche o "Atualizado Em"). `listarMeusProjetos` reaproveita as linhas de `syncOwnerRowsFromSheet` p/ marcar `pendente`/`atualizado_em` por projeto (sem coluna nova no SQLite). A tela "Meus Projetos" mostra aviso por linha pendente; a home tem selo de pendentes via `GET /api/meus-projetos/pendentes` (`contarPendentes`). Prazo de regularização (`PRAZO_LEGADO` em `meus-projetos.functions.ts`): **30/06/2026**.
