...
- **Ownership: owner edita, participante só visualiza** — em `meus-projetos.functions.ts`: **owner** = `responsavel_email` (quem submeteu); **participante** = está em `membros` mas não é owner. `temAcesso` = owner OU participante (leitura). **Só o owner (ou admin RPA) edita**; participante apenas visualiza. `listarMeusProjetos` marca o `papel` de cada item; `getMeuProjeto` retorna `papel` + `podeEditar` (owner/admin). Gate server-side definitivo: `submeterParaValidacao(body, email)` bloqueia (403) reenvio de quem não é owner/admin. Frontend: `meus-projetos.tsx` tem 4 filtros — **Todos** (padrão, owner+participante submetidos) · **Meus** (owner) · **Participo** (participante, só "Visualizar") · **Rascunhos**; `/editar/$id` redireciona não-owner para `/projeto/$id`; `/projeto/$id` é a tela **read-only** (memorial SEM R$, via `documentacao.conteudo`). **Mudança de owner → acionar a equipe RPA** (disclaimer via tooltip `(i)` no rótulo "Autoria" e nota no filtro Participo). Componentes: `components/info-tooltip.tsx`, `components/status-badge.tsx`. Legado pendente só conta/aparece para o owner (participante não pode regularizar). ⚠️ A edição seeda **todos** os campos da 1ª submissão, inclusive custo evitado (`custo_evitado`/`custo_evitado_itens` precisam estar no retorno de `getMeuProjeto` — o seed em `submeter.tsx` os lê).
