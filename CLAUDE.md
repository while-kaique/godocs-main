...
**Base das horas — padrão CLT 220h/mês (gate DETERMINÍSTICO):** todo raciocínio de horas economizadas usa **tempo útil** (1 mês ≈ **22 dias úteis ≈ 220h** de trabalho, jornada CLT), nunca horas de calendário → uma pessoa em tempo integral tem ~220h/mês de capacidade (teto de plausibilidade por linha). **Escopo: SÓ rotina manual real e mensal** — predicado único `aplicaConfirmacaoBaseHoras(ctx, saving)` em `orchestrator.ts` (`!ninguemFazia && !isPontual && temHorasAntes`), usado pelo prompt E pelo backend; **não** se aplica ao saving contrafactual ("ninguém fazia" = horas estimadas) nem ao pontual (total único não mapeia para "220h no mês"). Faz duas coisas: **(1) Confirmação obrigatória com botões Sim/Não — conduzida pelo SISTEMA (não pelo LLM):** o gate vive em `chat.functions.ts`/`enviarMensagem`. Estado em `saving.confirmacao_220h` (`null`→`'pendente'`→`'sim'`/`'nao'`), **gerenciado pelo backend** e **re-mesclado a cada turno** porque o orquestrador adota o `saving` ecoado pelo LLM (que não traz o campo). Quando o LLM tenta `preview`/`complete` no saving com a base ainda não confirmada (`== null`), o backend **troca o preview pela pergunta** (`type:"options"`, `["Sim","Não"]`, com o total real no texto) e mantém a fase em `saving` (`perguntaBase220`). No turno seguinte o backend interpreta a resposta (`interpretarRespostaBase220`: índice 1=Sim/2=Não ou texto) e injeta um **nudge `[SISTEMA]`** efêmero no histórico: **"Sim"** → manda gerar o preview; **"Não"** → manda **reconciliar** (o LLM explica o padrão e pede para reexpressar as horas em trabalho efetivo/mês, atualizando as `linhas` — sincronia linhas×memorial). Resposta ambígua → re-pergunta determinística (sem chamar o orquestrador). O prompt do saving (`baseHorasBlock`) instrui o LLM a **NÃO** fazer a pergunta e só reagir aos avisos `[SISTEMA]`. O gate dispara **uma vez** (null→pendente→respondido); depois de respondido não volta a disparar. **(2) Plausibilidade vs. 220h (via prompt):** se a economia de um cargo for alta/exagerada frente à capacidade (ex.: uma única atividade tirando ~50h/mês de alguém, ou um cargo economizando fração grande das 220h), **exige o detalhamento** de como as horas se acumulam (atividade × frequência × tempo, somando o total), reforçando a **composição das horas**; se a soma não fecha com o total, aponta a discrepância e ajusta as `linhas`. Botões Sim/Não = 2 opções; `OrchestratorResult.options` é `string[]` e o frontend (`step3-chat.tsx`) renderiza qualquer nº de `options` (não precisa ser 3).
