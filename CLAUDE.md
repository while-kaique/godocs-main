...
- **Rascunhos (Meus Projetos)** — `/submeter` persiste o rascunho em localStorage (`src/lib/submeter/draft-storage.ts`) e RETOMA o mesmo `projetoId` ao atualizar/voltar (não cria órfão); limpa ao submeter. `?retomar=<id>` reabre um rascunho específico (cross-device usa `GET /api/chat/historico/:id`). A tela `meus-projetos.tsx` tem abas **Submetidos** (padrão) e **Rascunhos** (secundária); rascunhos nunca vão para o Sheets (a IDA só roda na submissão).
