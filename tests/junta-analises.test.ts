...
const est = (saida: string, over: Record<string, unknown> = {}) => ({
  saida,
...
describe("juntarAnalises", () => {
  ...
  it("⚠️ a faixa 6-10 vence tudo: Pendente com flag, mesmo com as duas aprovando", () => {
    const j = juntarAnalises({
      impacto: imp("aprovar"),
      estrela: est("aprovar", { escape: true }),
    });
    expect(j.status).toBe("Pendente");
  ...
  it("⚠️ mas o time ainda VETA: reprovar mecânico e faixa 6-10 vencem a aprovação da mesa", () => {
    expect(juntarAnalises({ impacto: imp("aprovar"), estrela: est("reprovar") }).status).toBe(
      "Pendente",
    );
    expect(
      juntarAnalises({ impacto: imp("aprovar"), estrela: est("aprovar", { escape: true }) })
        .flag6a10,
    ).toBe(true);
  });

  it("em_validacao e isento da mesa contam como Pendente (vocabulário dela)", () => {
    expect(juntarAnalises({ impacto: imp("em_validacao"), estrela: est("humano") }).status).toBe(
      "Pendente",
    );
    expect(juntarAnalises({ impacto: imp("isento"), estrela: est("humano") }).concordam).toBe(true);
  });

  // ⚠️ A lista sai de `STATUS_FUNIL`, não é digitada: quando `Ajuste pedido` entrou (15/09/2026)
  // este teste quebrou por comparar contra três literais, e uma lista à mão volta a mentir na
  // próxima vez. `Pré-aprovado` fica de fora de propósito — a junta nunca o grava (é o estado de
  // ENTRADA do agente), e o teste abaixo cobra isso.
  it("nenhum caminho devolve status fora dos desfecho do funil", () => {
    ...
    for (const a of casos)
      for (const b of ["aprovar", "reprovar", "humano", "ajuste", "xpto"]) {
        ...
        expect([...STATUS_FUNIL]).toContain(j.status);
        expect(j.status).not.toBe("Pré-aprovado");
        expect(j.porques.length).toBeGreaterThan(0);
      }
  });

  /**
   * ⚠️ A regra 2b (o mérito tem UM dono: a mesa) foi medida em prod em 10/09/2026 sobre 9
   * projetos, e em 7 deles o time vinha com `ajuste`. Quando `ajuste` deixou de ser `Pendente`
...
describe("⚠️ a reprovação MECÂNICA da mesa também não é divergência", () => {
  ...
  it("a faixa 6-10 vence até a reprovação da mesa", () => {
    const j = juntarAnalises({
      impacto: imp("reprovar"),
      estrela: est("humano", { escape: true }),
    });
    expect(j.status).toBe("Pendente");
    expect(j.flag6a10).toBe(true);
  });
...
describe("fecharPendente: a condição é só sobre a MESA (o mérito tem um dono)", () => {
  ...
  it("e a mesa aprovando segue mandando: Aprovado, com o funil fechando ou não", () => {
    expect(
      juntarAnalises({ impacto: imp("aprovar"), estrela: est("ajuste"), fecharPendente: true })
        .status,
    ).toBe("Aprovado");
  });
});
...
describe("⚠️ as DUAS travas do fecharPendente (10/09/2026) — medidas em prod", () => {
  it("nota ≥ 1 SUSTENTA o projeto: o funil aprova em vez de contradizer a própria nota", async () => {
    ...
    expect(
      juntarAnalises({
        impacto: { veredito: "em_validacao" },
        estrela: { saida: "ajuste", estrela: 5 },
        fecharPendente: true,
      }).status,
    ).toBe("Aprovado");
  });
  ...
  it("⚠️ FALLBACK do cérebro (nota 0 não avaliada) não sustenta e também não reprova", async () => {
    ...
    expect(
      juntarAnalises({
        impacto: { veredito: "em_validacao" },
        estrela: { saida: "ajuste", estrela: 3, avaliada: false },
        fecharPendente: true,
      }).status,
    ).toBe("Pendente");
  });
