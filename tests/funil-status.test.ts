...
describe("statusDoFunil — o desfecho do time vira status", () => {
  ...
  /**
   * ⚠️ REGRESSÃO REAL (15/09/2026). Enquanto `ajuste` era achatado em `Pendente`, o projeto
   * voltava para a fila sem que ninguém soubesse que havia um pedido — e o cron o reavaliava
   * depois de 72h, concluía `ajuste` de novo, e assim por diante. Medido em prod no
...
describe("resumirPontos — recorta o parecer, não reescreve", () => {
  ...
  it(`no máximo ${PONTOS_MAX} pontos e cada um até ${PONTO_MAX} chars`, () => {
    const p = resumirPontos(
      ["a: " + "x".repeat(400), "b: dois.", "c: três.", "d: quatro."].join("\n"),
    );
...
describe("agenteDecideFunil — env em runtime", () => {
  it("default OFF; 1/true/sim/on ligam; 0 desliga", () => {
    ...
    try {
      delete process.env.AGENTE_DECIDE_FUNIL;
      expect(agenteDecideFunil()).toBe(false);
      for (const v of ["1", "true", "sim", "on", " ON "]) {
        process.env.AGENTE_DECIDE_FUNIL = v;
        expect(agenteDecideFunil()).toBe(true);
      }
      process.env.AGENTE_DECIDE_FUNIL = "0";
      expect(agenteDecideFunil()).toBe(false);
    } finally {
      if (antes === undefined) delete process.env.AGENTE_DECIDE_FUNIL;
      else process.env.AGENTE_DECIDE_FUNIL = antes;
    }
  });
});
