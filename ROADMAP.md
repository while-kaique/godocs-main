...
**13/08 — ✅ PR #259 MERGEADO: o `main` voltou a ser fonte segura de deploy, e prod = staging = `main`.** Merge `71e2821` (`feat/espelho-e-perf-navegacao` → `main`), o que fecha a **última pendência grave do repo**: o espelho da planilha, os 2 commits de perf do Kaique e a ficha instantânea estavam em PRODUÇÃO e **não** no `main` — deploy a partir do `main` derrubava os três de uma vez. ⚠️ **Provei a igualdade dos 3 lados sem tocar em nenhum app** (a checagem que evita o 7º atropelo): entry do `index.html` = `index-BHxUfRoT.js` em prod (v247), staging (v156) e no build do `main`; `worker.js` = **1.034.991 bytes** nos três; os **22** arquivos do `dist/` presentes com tamanho idêntico nos 2 `assetManifest` (0 ausentes, 0 divergentes). Hash do Vite **é** conteúdo, então entry igual = fonte igual. **1443 testes verdes** no `main`, `worker.js` sem drift após rebuild. Isso prova o **bundle armazenado, não o runtime** — este já tinha sinal verde no deploy do dia. **Repo local sincronizado:** principal no `main` = `origin/main`, 13 worktrees removidos, 23 branches locais mergeadas apagadas; sobrou `docs/plano-chat-so-pre-aprovacao` (1 commit **só de docs**, código já no `main`) e as 3 remotas fora do `main` seguem intocadas. ⚠️ **A revisão dos 3 revisores segue PENDENTE** — o merge saiu por pedido direto do operador, não por gate cumprido.

**Próximo:** 🟢 **rodar os 3 revisores de contexto fresco** sobre o espelho e a ficha instantânea (`.review-status`/`.quality-status` **ausentes** desde que as 2 frentes nasceram; o `/ggsd:ship` barra qualquer envio futuro até existirem, e hoje esse código está **em produção sem revisão independente**) — o diff a revisar é o do PR #259, `git diff 05e6692...71e2821`. _Em paralelo, segue de pé:_ 🟢 **renovar o `E2E_COOKIE`** do `.env` (vencido — bloqueia o harness E2E inteiro, não só a validação deste deploy) e **validar no navegador** o que subiu. _Em paralelo, segue de pé:_ 🟡 **olho humano de LÍDER — 2 confirmações, 1 clique cada:** o Estevão (ou o Lucas) reabrir "Ler a documentação completa" e ver a doc; e acompanhar a próxima submissão real de um liderado para **provar o wiring do aviso** (único pedaço da D26 nunca exercitado ponta a ponta), conferindo no `GET` do endpoint do Gomoon. 🟡 **O Luis validar no navegador** a ferramenta editável na staging (trocar a ferramenta em `/editar/<id>`, reenviar, conferir a coluna "Ferramenta" na aba `STAGING`; testar um legado e um projeto de escopo externo). _Depois:_ decidir sobre o **backfill dos 35** (SUSPENSO pela decisão "só com os novos submetidos") · 3 perguntas ao João Victor (§16 de `docs/integracao-gomoon-chat.md`). 🆕 **Pedido novo do Lucas (não planejado):** o card da fila precisa mostrar o que a pessoa marcou em **"Alguém já fazia?"** (`alguem_fazia` hoje não vai no payload do card) — mudança de payload + UI, começar por `/ggsd:plan`; 2 perguntas de escopo para ele no `docs/NEXT-SESSION.md`.
