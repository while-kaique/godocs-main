...
**Próximo:** 🔴 **mesclar o PR #235** (`/ggsd:ship`) — o disparo retroativo FOI FEITO em 06/08 (prod restaurada · backfill de **36 projetos / 28 líderes** · aba "Relação Líder-Liderado" regravada batendo com o payload · disparo real **202, 0 falhas**, entrega conferida no log do Gomoon), e o aviso IMEDIATO na submissão está no ar e validado por dry-run com `projetoId`. O que sobra é a **trava**: enquanto a branch não estiver no `main`, o próximo deploy de outra frente derruba a feature de novo — agora com 36 filas abertas e 28 líderes já avisados. O Luis foi perguntado e ainda não respondeu. _Em seguida:_ **provar o wiring do `submeterParaValidacao` numa submissão real** (único pedaço da D26 nunca exercitado — acompanhar pelo `GET ?email=` do Gomoon) e o Luis conferir a aba "Aprovações Pendentes por Líder". ⚠️ 2 divergências entre as fontes, achadas no retroativo: id de legado em CAIXA diferente entre planilha (maiúsculas) e SQLite (minúsculas), que some da lista sem erro; e o `reabrir` não filtra `[E2E-…]` (corrigido nos scripts, commit `2c577ec`). ⚠️ 3 perguntas ao João Victor seguem abertas (§16 de `docs/integracao-gomoon-chat.md`).
