...
_(Frente de latência — **Fatia C (reordenar o wizard, doc paralela + refino no fim)** 🟡 **CÓDIGO COMPLETO + na STAGING 31/08**, atrás da flag `REORDER_DOC_FINAL` (default OFF). Worktree `~/godocs-wt-fatia-c`, branch `feat/fatia-c-reorder-wizard`. **Destravar prod (G+F+B): G+B CODADOS 31/08** (commit `4905faa`, suíte 2353 verde) — G (cabeçalho doc×memorial: guarda de transição alargada p/ qualquer fase não-doc→"doc" no reorder, `deveIrParaRefinoDoc` puro) + B (loader rotulado de receita). [docs/plans/fatia-c-destravar-prod.md](docs/plans/fatia-c-destravar-prod.md). F (`reasoning_effort=low`) é medição na staging. **Próximo:** deploy staging → Luis valida G/B + mede F → prod + merge. D/E/C/A (frente sensível aos gates) ficam à parte.)_
