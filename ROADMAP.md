...
**Próximo:** **o Luis conferir a aba "Relação Líder-Liderado" (prod) e, se ok, deployar a D20 na staging `edf400b4`** — junto com **conferir a coluna "Pré-status" da tabela + a seção "Pré-aprovação do líder" na ficha (de preferência num projeto com "não" no checklist)**. Sobre o Gomoon: **mandar ao João Victor os adendos §13/§14 do contrato v2** (aceitar `mensagem.texto` no diário + criar o endpoint `/api/godocs/anuncio`, honrando `ambiente:"staging"` também nele) e, quando ele implementar, **validar as 2 mensagens na staging** (`{"dry":true}` → `{"dry":false}`). Segue de pé pedir a ele **trocar o destinatário de teste do modo staging para o Luis** (no modo staging os e-mails do payload são ignorados, então não temos como fazer a DM chegar nele). Em
