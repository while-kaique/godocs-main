...
**Próximo:** **provar o disparo ponta a ponta numa submissão real** — 1 submissão na staging (ou acompanhar a 1ª de prod pelo `GET` no endpoint do Gomoon com `?email=`) — e só então **abrir o PR da branch `worktree-plano-aprovacao-lider-teamguide` com `/ggsd:ship`**. ⚠️ 3 perguntas ao João Victor seguem abertas (§16 de `docs/integracao-gomoon-chat.md`): descarte de item em retry quando chega POST novo, volume de N POSTs, e confirmar que **ele** dispara o anúncio à mão (nada no GoDocs chama `/api/godocs/anuncio`).
