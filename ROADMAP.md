...
**Próximo:** **o Luis validar as 2 DMs do líder na staging** (disparadas 06/08 sem a linha de link — o botão "Abrir a fila" do cartão é o único caminho) e, com o OK, **abrir o PR da branch `worktree-plano-aprovacao-lider-teamguide` com `/ggsd:ship`**. ⚠️ O rollout para PROD segue travado pela diretoria e depende de 2 coisas que ainda não existem lá: o secret **`GOMOON_TOKEN`** e o cron `0 12 * * 1-5` → `/api/cron/notificar-lideres`. O **anúncio global saiu do GoDocs** (D24): quem guarda o texto e dispara é o Gomoon — o combinado está no §15.1 de `docs/integracao-gomoon-chat.md`.
