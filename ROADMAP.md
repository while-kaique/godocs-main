...
**Próximo:** 🔴 **deploy — staging `edf400b4` → validar `/projeto/$id` como líder → prod `674a3710`** (regra 13). O `main` agora contém as 3 frentes (pré-aprovação + coluna do líder + 403), então este deploy é o que **sincroniza os 2 apps com o repo** e fecha o ciclo dos atropelos. _Depois:_ **provar o wiring do `submeterParaValidacao` numa submissão real** (único pedaço da D26 nunca exercitado) · decidir sobre o **backfill dos 35** (SUSPENSO pela decisão "só com os novos submetidos") · 3 perguntas ao João Victor (§16 de `docs/integracao-gomoon-chat.md`).
