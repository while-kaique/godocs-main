...
**Próximo:** 🟡 **olho humano de LÍDER — 2 confirmações, 1 clique cada:** o Estevão (ou o Lucas) reabrir "Ler a documentação completa" e ver a doc; e acompanhar a próxima submissão real de um liderado para **provar o wiring do aviso** (único pedaço da D26 nunca exercitado ponta a ponta), conferindo no `GET` do endpoint do Gomoon. _Depois:_ decidir sobre o **backfill dos 35** (SUSPENSO pela decisão "só com os novos submetidos") · 3 perguntas ao João Victor (§16 de `docs/integracao-gomoon-chat.md`).
