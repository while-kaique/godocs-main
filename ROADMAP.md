...
**Próximo:** **o Luis conferir a aba "Relação Líder-Liderado" (prod) e, se ok, deployar a D20 na staging `edf400b4`** — junto com **conferir a coluna "Pré-status" da tabela + a seção "Pré-aprovação do líder" na ficha (de preferência num projeto com "não" no checklist)**. Sobre o Gomoon, o que resta é do lado deles: **pedir ao João Victor (a) trocar o destinatário de teste do modo staging para o Luis** (é ele quem escolhe — no modo staging os e-mails do payload são ignorados) **e (b) disparar o anúncio da feature**, que é broadcast do Gomoon e **não passa pela nossa API**. Em
