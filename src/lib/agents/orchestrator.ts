...
- AO DETECTAR projeção, PARE e pergunte UMA vez, direto: "${perguntaUnica}"
  • Se o usuário CONFIRMAR que já acontece e foi MEDIDO (peça a base: há quanto tempo roda e como mediram), ${oQueEscrever}
  • Se for apenas expectativa/estimativa, NÃO gere o preview. Explique que o GoDocs registra ganhos JÁ realizados e oriente a (a) voltar quando o ganho estiver medido, ou (b) submeter como PROJETO ESPECIAL se for caso de alto impacto e difícil mensuração. NÃO monte memorial com números projetados. ⚠️ NUNCA "resolva" isso escrevendo a ressalva DENTRO do memorial ("a taxa não é histórico medido, é premissa de piso") e gerando o preview mesmo assim — foi exatamente o bug de origem: a planilha recebe o valor como ganho apurado e a ressalva não desconta nada.${
    modo === "saving"
      ? `
