...
export function getPromptRegistry(): PromptEntry[] {
  return [
    ...
    },
    {
      ...
      filePath: 'src/lib/agents/orchestrator.ts',
      fase: 'saving',
      description: 'Fluxo de saving DEDICADO ao CUSTO EVITADO PURO (alguem_fazia="externo"): o usuário marcou no formulário que ninguém fazia o trabalho internamente, que a automação ELIMINOU um gasto externo (contrato/serviço/licença) e que NÃO há trabalho manual adicional — então o ganho é 100% o custo externo eliminado, sem horas de pessoas a calcular. buildSavingPrompt DELEGA para cá quando ctx.alguem_fazia === "externo". O prompt PROÍBE perguntar sobre horas/rotina/equivalente manual e proíbe criar a seção "Saving de Pessoas" (o array linhas fica vazio). A missão é enxuta (no máximo 1-2 perguntas): confirmar que a automação realmente substituiu o que o contrato/serviço fazia e que o gasto foi DE FATO cancelado/reduzido na prática (portão GANHO REAL × PROJETADO — "já foi cancelado ou ainda vai acontecer?"; se for projeção, não gera preview). O memorial tem só Contexto → Contratos/Serviços Evitados → Resumo (sem horas e sem R$ no texto visível; o valor em R$ vive em custo_evitado_reais, preenchido pelo formulário e PRESERVADO). Resolve o caso "contrato terceirizado cancelado" (ex.: Portal de Reembolsos) sem a dupla contagem que o caminho contrafactual de horas causava (176h × tarifa interna + o valor do contrato).',
      llmParams: { temperature: 0.4, maxTokens: 4096, modelTier: 'fast', jsonMode: true },
