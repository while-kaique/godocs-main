...
export function getPromptRegistry(): PromptEntry[] {
  return [
    ...
    // ── Analisador (1) ──
    {
      ...
      fase: null,
      description: 'Análise holística pré-submissão. Avalia toda a submissão com 10 critérios fixos + 2-3 dinâmicos por projeto. Classifica complexidade (automação/inteligência/autonomia). Gera parecer, resumo e pontuação. Considera também o TEXTO EXTRAÍDO dos arquivos que a pessoa enviou (documentacao_enviada_usuario). Roda em background após o envio — incl. PROJETOS ESPECIAIS: para eles o analisador NÃO decide status (validação é humana), mas classifica complexidade e dá um PARECER avaliando criticamente se o projeto realmente se enquadra como especial (alto impacto + difícil mensuração objetiva) ou se é uma automação padrão mal rotulada que deveria ir como saving/receita.',
