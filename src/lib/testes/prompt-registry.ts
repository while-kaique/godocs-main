...
export function getPromptRegistry(): PromptEntry[] {
  return [
    ...
    },
    {
      ...
      filePath: 'src/lib/agents/orchestrator.ts',
      fase: 'saving_preview',
      description: 'Revisão do memorial de saving. Mesma mecânica de aprovação/ajuste do doc_preview. Se aprovado e há receita pendente, transita para fase receita; senão, marca completo. REGRA ANTI-ZERO: NUNCA emite complete se economia_horas_mes <= 0 — força volta à coleta.',
      llmParams: { temperature: 0.4, maxTokens: 4096, modelTier: 'fast', jsonMode: true },
    ...
    },
    {
      ...
      description: 'Revisão do memorial de receita. Aprovação ou ajuste. Se aprovado, marca como completo — última fase do fluxo. REGRA ANTI-ZERO: NUNCA emite complete se valor_ganho_mensal <= 0 — força volta à coleta.',
      llmParams: { temperature: 0.4, maxTokens: 4096, modelTier: 'fast', jsonMode: true },
    ...
    // ── Analisador (1) ──
    {
      ...
      fase: null,
      description: 'Análise holística pré-submissão. Avalia toda a submissão com 10 critérios fixos + 2-3 dinâmicos por projeto. Classifica complexidade (automação/inteligência/autonomia). Gera parecer, resumo e pontuação. Roda em background após o envio para triagem.',
