...
async function execContando(sql: string, params: unknown[] = []): Promise<number | null> {
  const resultado = (await getDb().exec(sql, params)) as
    | { rowsWritten?: unknown }
    | null
    | undefined;
...
export async function getProjetoWithRelations(id: string) {
  ...
  const validacoes = await queryAll<ValidacaoRow>("SELECT * FROM validacoes WHERE projeto_id = ?", [
    id,
  ]);
...
export function getDocumentacaoConteudo(projetoId: string) {
  return queryOne<{ conteudo: string }>(
    "SELECT conteudo FROM documentacao WHERE projeto_id = ? LIMIT 1",
    [projetoId],
  );
}
...
export function updateProjeto(id: string, fields: Record<string, unknown>) {
  ...
  return exec(`UPDATE projetos SET ${sets}, updated_at = ? WHERE id = ?`, [
    ...values,
    nowISO(),
    id,
  ]);
 ...
 */
export async function deleteChatMessagesAfterFaseMarker(
  projetoId: string,
  fase: "saving" | "receita",
) {
  const rows = await queryAll<{ id: string; role: string; content: string }>(
    "SELECT id, role, content FROM chat_messages WHERE projeto_id = ? ORDER BY created_at",
    [projetoId],
  );
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (parsed.type === "complete" && parsed.fase === fase) {
        startIdx = i;
        break;
      }
    } catch {
      /* não-JSON (ex.: role 'doc') — ignora */
    }
  }
  if (startIdx < 0) {
    for (let i = 0; i < rows.length; i++) {
...
export function getFormEventsByProjeto(projetoId: string) {
  return queryAll<FormEventRow>(
    "SELECT * FROM form_events WHERE projeto_id = ? ORDER BY created_at",
    [projetoId],
  );
}
...
export async function upsertDocumentacao(projetoId: string, conteudo: unknown) {
  ...
  if (existing) {
    await exec("UPDATE documentacao SET conteudo = ?, updated_at = ? WHERE projeto_id = ?", [
      jsonStr,
      now,
      projetoId,
    ]);
    ...
    const id = generateId();
    await exec(
      "INSERT INTO documentacao (id, projeto_id, conteudo, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
      [id, projetoId, jsonStr, now, now],
    );
  }
}
...
export async function insertValidacao(data: {
  ...
  `,
    [
      ...
      data.resultado,
      data.parecer,
      data.criterios ? JSON.stringify(data.criterios) : null,
...
export function finalizeEmailLote(id: string, status: "concluido" | "erro" | "cancelado") {
  return exec("UPDATE email_lotes SET status = ?, updated_at = ? WHERE id = ?", [
    status,
    nowISO(),
    id,
  ]);
...
/**
 * Histórico de status de VÁRIOS projetos numa consulta só — o lote que semeia as fichas da
 * página visível do `/dashboard`. Uma chamada por projeto seria o erro que já derrubou o
 * Investigador (round-trip por linha dentro de um laço).
 *
 * ⚠️ O `LIMIT` global é proposital: sem ele, um projeto com histórico longo comeria a cota
 * dos outros. `POR_PROJETO` × ids é folgado para o que a ficha mostra.
 */
export async function getAdminStatusLogsPorIds(
  projetoIds: string[],
): Promise<Map<string, AdminStatusLogRow[]>> {
  const ids = [...new Set(projetoIds.map((i) => i.trim().toLowerCase()).filter(Boolean))];
  const out = new Map<string, AdminStatusLogRow[]>();
  if (ids.length === 0) return out;
  const POR_PROJETO = 20;
  const placeholders = ids.map(() => "?").join(", ");
  const linhas = await queryAll<AdminStatusLogRow>(
    `SELECT * FROM admin_status_log WHERE LOWER(projeto_id) IN (${placeholders})
      ORDER BY created_at DESC LIMIT ?`,
    [...ids, ids.length * POR_PROJETO],
  );
  for (const l of linhas) {
    const k = String(l.projeto_id ?? "").toLowerCase();
    const atual = out.get(k);
    if (atual) {
      if (atual.length < POR_PROJETO) atual.push(l);
    } else {
      out.set(k, [l]);
    }
  }
...
export function getUserRole(userId: string, role?: string) {
  if (role) {
    return queryOne<UserRoleRow>("SELECT * FROM user_roles WHERE user_id = ? AND role = ?", [
      userId,
      role,
    ]);
  }
...
function truncateBody(raw: string | null | undefined): string | null {
  ...
  return (
    raw.slice(0, API_LOG_BODY_LIMIT) +
    "\n…[truncado — " +
    raw.length.toLocaleString("pt-BR") +
    " chars]"
  );
}
...
const API_LOG_LIGHT_COLS =
  "id, projeto_id, endpoint, method, duration_ms, status_code, error, request_size, response_size, created_at";

export function getApiLogsByProjeto(projetoId: string) {
  return queryAll<ApiLogRow>(
    `SELECT ${API_LOG_LIGHT_COLS} FROM api_logs WHERE projeto_id = ? ORDER BY created_at DESC`,
    [projetoId],
  );
}
...
export function getApiLogsRecent(limit = 200) {
  return queryAll<ApiLogRow>(
    `SELECT ${API_LOG_LIGHT_COLS} FROM api_logs ORDER BY created_at DESC LIMIT ?`,
    [limit],
  );
}

export function getApiLogById(id: string) {
  return queryOne<ApiLogRow>("SELECT * FROM api_logs WHERE id = ?", [id]);
}
...
export function cleanupOldApiLogs(daysToKeep = 30) {
  return exec("DELETE FROM api_logs WHERE created_at < datetime('now', '-' || ? || ' days')", [
    daysToKeep,
  ]);
}
...
export function getSyncRunsRecentes(limite = 20) {
  return queryAll<SyncRunRow>("SELECT * FROM sync_runs ORDER BY iniciado_em DESC LIMIT ?", [
    limite,
  ]);
}
...
export function cleanupOldSyncRuns(daysToKeep = 7) {
  return exec("DELETE FROM sync_runs WHERE iniciado_em < datetime('now', '-' || ? || ' days')", [
    daysToKeep,
  ]);
}
